...
export const displayNameFn = (school?: SchoolInterface) => {
  return [undefined, "1", "2", 1, 2].includes(
    school?.name_display_stats as unknown as string,
  );
};
...
export default function resultsUiStats(
  exam: ExamInterface,
  competences: CompetenceInterface[],
  results: ExamResultInterface[],
  statResults: ExamResultInterface[],
            ...
            {competences &&
              competences.map((s) => {
                const spans = s.subjects?.reduce((a, b) => {
                  return (
                    a + b.courses?.length + (school?.sub_total_display ? 1 : 0)
                  );
                }, 0);

                return (
                  <th key={s._id} colSpan={spans}>
                    {s.slug || s.name}
                  </th>
                );
              })}
      ...
      <div style={{ marginTop: "60px", fontSize: "20px" }}>
        <table>
          ...
          <tr>
            ...
            <td style={{ color: "red" }}>
              {(
                (getTotal(statsResults[statsResults.length - 1]) / points) *
                20
              ).toFixed(2)}
            </td>
          </tr>
...
const reducer = (previousValue: any, currentValue: any) =>
  getFloat(previousValue ?? 0) + getFloat(currentValue ?? 0);
...
export function ExamResult({
  ...
  return (
    <tr>
      ...
      {displayName && (
        <td>
          {school?.name_display_stats == 1
            ? result?.student?.name?.split(" ")[0]
            : result?.student?.name}
        </td>
      )}
      {competences &&
              ...
              <>
                {subject.courses?.map((course) => {
                  return (
                    course._id && (
                      <td key={course._id}>
                        {result[`subject_${course._id}`]}
                      </td>
                    )
                  );
                })}
                {school?.sub_total_display && (
