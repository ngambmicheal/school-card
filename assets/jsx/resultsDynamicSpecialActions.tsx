...
const getAppreciation = (
  ...
  if (total == 20) {
    ...
    if (value < 15) return displayName ? "SIA" : "ECA";
    if (value < 18) return displayName ? "Acquired" : "A";
    if (value < 21) return displayName ? "Expert" : "A+";
  }
  if (total == 30) {
    if (value <= 15) {
      if (comT.indexOf(competenceName) < 0) comT.push(competenceName);
...
export default function resultsDynamicSpecialActions(
  ...
  return (
    <>
      ...
      <table className="table1" style={{ fontSize: "20px" }}>
        <thead>
          <tr>
            <th colSpan={2}>MATIERES</th>
            <th>SUR</th>
            {exams.map((exam, index) => {
              return <th>{exam.name.substr(0, 4)}</th>;
            })}
            <th>NOTES</th>
            <th colSpan={2}>COTATIONS</th>
          </tr>
        </thead>
        <tbody>
            ...
            return (
              <>
                <tr>
                  ...
                  <td>
                    {getAppreciation(
                      results[`subject_${subject._id}`] ?? 0,
                      exams[0][`point_${subject._id}`],
                      false,
                      subject.name,
                    )}
                  </td>
      ...
      <table style={{ fontSize: "25px", width: "100%" }} className="table1">
        <tr>
          <th>TOTAL </th>
          <th>
            {getFloat(totalMarks)} / {totalPoints}
          </th>
          <th>COTES</th>
