...
export function CreateStudentModal({modalIsOpen, closeModal, save, class_id, totalUsers}:CreateStudentModalProps){
    ...
    function handleChange(e:any) {
      ...
      useEffect(() => {
        api.getClasses().then(({data:{data}}:any) => {setClasses(s => data)})
      }, [])
        ...
        <div>
          ...
          >
            ...
            <h2 >Add Student</h2>
                ...
                <div className='form-group'>
                    <label>Class</label>
                    <select className='form-control' name='class_id' value={student?.class_id} onChange={handleChange}>
                    {classes?.map(classe => <option value={classe._id}>{classe.name}</option> )}
                    </select>
                </div>
