...
export default async function handler(
  ...
  res: NextApiResponse<any>
    ...
    const term:TermInterface = await termSchema.findOne({_id:term_id}).populate({path:'class', model:classeSchema}).populate({path:'exams', model:examSchema})
    const totalResults = await examResultSchema.find({term_id, th:true}).populate({path:'student', model:studentSchema})
    ...
    totalResults.map(async (results) => {
        ...
        let html = ReactDOMServer.renderToStaticMarkup(thEn(results, term))
        html+=`
        <style>
            .b{
                color : #020066,
