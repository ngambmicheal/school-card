...
import AttestationMaternelleEn from '../../../../assets/attestation/maternelle_en';
...
export default async function handler(
  ...
  res: NextApiResponse<any>
    ...
    const term:ExamInterface = await examSchema.findOne({_id:exam_id}).populate({ path: "class_id", model: classeSchema, populate: { path: "school", model: schoolSchema }})

    const totalResults = await examResultSchema.find({exam_id, th:'true'}).populate({path:'student', model:studentSchema})
    ...
    totalResults.map(async (results) => {
        ...
        let html = ReactDOMServer.renderToStaticMarkup(AttestationMaternelleEn(results, term))
        html+=`
        <style>
            .b{
                color : #020066,
