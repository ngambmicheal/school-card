...
export default async function handler(
  ...
  const term: ExamInterface = await examSchema
    .findOne({ _id: exam_id })
    .populate({ path: "class_id", model: classeSchema, populate: { path: "school", model: schoolSchema }})
    .populate({ path: "exams", model: examSchema });

  const totalResults = await examResultSchema
    .find({ exam_id, th: "true" })
  ...
  totalResults.map(async (results) => {
    ...
    let html = ReactDOMServer.renderToStaticMarkup(attestationFr(results, term));
    html += `
        <style>
            .b{
                color : #020066,
