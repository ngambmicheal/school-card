...
export default function examDetails() {
  ...
  const printResults = () => {
    window.open(
      `/api/exams/dynamic/${term?.report_type?.toLocaleLowerCase()}?term_id=${examId}`,
      "_blank"
    );
  };
  ...
  const printStats = () => {
    window.open(
      `/api/exams/dynamic/${term?.report_type?.toLocaleLowerCase()}-stats?term_id=${examId}`,
      "_blank"
    );
  };
  ...
  const printTD = () => {
    window.open(
      `/api/exams/td/${term?.report_type?.toLocaleLowerCase()}?term_id=${examId}`,
      "_blank"
    );
  };
    ...
    <>
      ...
      {resultsCsv.length && (
        <CSVLink
          ...
          filename={`statistics-${term?.class?.name}-${term?.name}.csv`}
        >
          Telecharcher Csv
        </CSVLink>
      )}
  ...
  return (
    <tr>
      ...
      <th>
        {" "}
        <Link
          href={`/api/exams/results-special/dynamic-print?term_id=${res.term_id}&student_id=${res.student?._id}`}
        >
          Imprimer
        </Link>{" "}
