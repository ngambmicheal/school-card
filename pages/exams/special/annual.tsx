...
export default function termDetails(){
    ...
    useEffect(() => {
        if(term?._id){
            ...
            api.updateExam(term._id, term).then(({data:{data}} : any) => {
                //setExam(data)
            })
        }
    }, [term])
...
export function ExamResult({result, subjects, points, deleteResult}:{subjects:SubjectInterface[], result:ExamResultInterface|any, points:number, deleteResult:(resultId:string)=>void }){
   ...
   return  <tr>
        ...
        <th> <Link href={`/api/terms/results-special/dynamic-print?annualExam_id=${res.annualExam_id}&student_id=${res.student?._id}`}>Imprimer</Link> | <a href='javascript:void(0)' onClick={() =>deleteResult(res._id)}> Delete</a> </th>
    </tr>
}