            ...
            {competences &&
              competences.map((s) => {
                return (
                  <th
                    key={s._id}
                    colSpan={(s.subjects?.length || 0) * nurseryActs.length}
                  >
                    {" "}
...
export function ExamResult({
  ...
  return (
    <tr>
      ...
      <th>
        {" "}
        <Link href={`/exams/nursery/print?_id=${res._id}`}>
          Imprimer
        </Link> | <a onClick={() => deleteResult(res._id)}> Delete</a>{" "}
      </th>
    </tr>
  );
}
