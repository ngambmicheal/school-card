            ...
            {competences &&
              competences.map((s) => {
                return (
                  <th
                    key={s._id}
                    colSpan={(s.subjects?.length || 0) * act.length}
                  >
                    {" "}
                    {s.slug?.substring(0, 40) || s.name}{" "}
                  </th>
                );
              })}
          </tr>
...
const reducer = (previousValue: any, currentValue: any) =>
  parseFloat(parseFloat(previousValue ?? 0).toFixed(2)) +
  parseFloat(parseFloat(currentValue ?? 0).toFixed(2));
...
export function ExamResult({
  ...
  return (
    <tr>
      ...
      <th>
        {" "}
        <Link href={`/exams/mat/print?_id=${res._id}`}>Imprimer</Link> |{" "}
